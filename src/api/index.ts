...
export const playlistTrackAll = (params: { id: number; offset?: number }) =>
  httpGet(
    `/playlist/track/all?id=${params.id}&limit=30&offset=${((params.offset || 1) - 1) * 30}`,
  );
//获取歌单详情
...
export const mvUrl = (id: string | undefined) =>
...
//获取 mv 数据
export const mvDetail = (mvid: string | undefined) =>
...
export const allMV = (params: MVParams) =>
  httpGet<ResultData>(
    `/mv/all?limit=${params.limit || 30}&offset=${(((params.offset || 1) as number) - 1) * 30}`,
    {
      area: params.area,
      order: params.order,
    },
  );
export const simiMV = (id: string | undefined) =>
  httpGet(`/simi/mv?mvid=${id}`);
