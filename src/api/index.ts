...
export const cloudsearch = (params: SearchParams) =>
  httpGet<ResSearch>(
    `cloudsearch?keywords=${encodeURIComponent(
      params.kw || '',
    )}&limit=30&offset=${((params.offset || 1) - 1) * 30}&type=${params.type || 1}`,
  );
//获取音乐连接
...
export const videoSearch = (params: VideoSearchParams) =>
  httpGet<ResVideoSearch>(`video/search?name=${encodeURIComponent(params.name)}`);
// 查看动漫详情
export const videoChapter = (id: number) => httpGet<ResVideoSearch>(`video/chapter?id=${id}`);
// 1. 二维码 key 生成接口
export const loginQrKey = () => httpGet<ResultData>('/login/qr/key');
// 2. 二维码生成接口
export const loginQrCreate = (key: string) =>
  httpGet<ResultData>(`/login/qr/create?key=${encodeURIComponent(key)}&qrimg=true`);
// 3. 二维码检测扫码状态接口
...
// 每日歌单
export const recommendResource = () => httpGet<ResRecommendResource>('/recommend/resource');
// 喜欢音乐
...
export const playlistTrackAll = (params: { id: number; offset?: number }) =>
  httpGet(`/playlist/track/all?id=${params.id}&limit=30&offset=${((params.offset || 1) - 1) * 30}`);
//获取歌单详情
export const playlistDetail = (id: number) => httpGet<ResPlaylistDetail>(`/playlist/detail?id=${id}`);
//获取歌词
export const lyric = (id: number | string) => httpGet<Reslyric>(`/lyric?id=${id}`);
...
export const topPlaylist = (params: pages) =>
  httpGet<{ playlists: [] }>(
    `/top/playlist?limit=30&offset=${(((params.offset || 1) as number) - 1) * 30}&cat=${params.cat}`,
  );
//歌单分类
...
export const commentMusic = (params: CommentMVParams) =>
  httpGet<CommentMVPOJO>(
    `/comment/music?id=${params.id}&limit=${params.limit || 30}&offset=${(((params.offset || 1) as number) - 1) * 30}`,
  );
// 歌单 评论
...
export const artistList = (params: CommentMVParams) =>
  httpGet<{ artists: [] }>(
    `/artist/list?limit=${params.limit || 30}&offset=${(((params.offset || 1) as number) - 1) * 30}`,
  );
/**
 * @description 歌手详情
 */
// 获取歌手详情
export const artistDetail = (id: number) => httpGet<ResultData>('/artist/detail', { id });
// 获取歌手描述
export const artistDesc = (id: number) => httpGet<artistDescModel>('/artist/desc', { id });
// 获取歌手单曲
export const artists = (id: number) => httpGet<artistDescModel>('/artists', { id });
// 获取歌手 MV
